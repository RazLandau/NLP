...
def evaluate_verbose(model, dataset, iter_lim=10, shuffle=True):
    ...
    is_sentence_samples = dataset.sample_type == 'sentence'
    ...
    if is_sentence_samples:
        for i, (M_s, y_true) in enumerate(g):
            if iter_lim is not None and i >= iter_lim:
                break
            M_s, y_true = M_s.to(device), y_true.to(device)
            y_logits, A = model(M_s)
            _, y_pred = torch.max(y_logits.data, 2)

            correct += (y_pred == y_true).sum()
            total += y_pred.size()[0] * y_pred.size()[1]

            M_s = M_s.cpu().numpy()
            for j in range(M_s.shape[0]):
                for k in range(M_s.shape[1]):
                    s = list(map(html.escape, tokens_vocab.decode_list(M_s[j])))
                    sentence_rows.append(s)
                    q_tokens.append(html.escape(tokens_vocab.decode(M_s[j][k])))
            v_qs.append(np.array([range(M_s.shape[1]), ] * M_s.shape[0]).flatten())
            y_true_np = y_true.cpu().numpy().flatten()
            y_trues.append(y_true_np)
            y_preds.append(y_pred.cpu().numpy().flatten())
            As.append(torch.flatten(A.detach().cpu(), end_dim=1).numpy())

            probs = nn.Softmax(dim=1)(torch.flatten(y_logits, end_dim=1)).detach().cpu().numpy()
